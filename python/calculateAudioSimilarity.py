...
def highPassFilter(fouriermatrix, threshold):
    ...
    result = numpy.zeros([length, 44100])
...
def calculateSimilarity(yourMusic, srcMusic):
    ...
    #各行ごとに真似したい側ノルムの作成
    normYourMusic = numpy.zeros([lenYourMusic, 1])
    for countNormYourMusic in range(0, lenYourMusic):
        normYourMusic[countNormYourMusic,0] = numpy.linalg.norm(yourMusic[countNormYourMusic,:])

    #各行ごとにサンプル側ノルムの作成
    normSrcMusic = numpy.zeros([lenSrcMusic, 1])
    for countNormSrcMusic in range(0, lenSrcMusic):
        normSrcMusic[countNormSrcMusic,0] = numpy.linalg.norm(srcMusic[countNormSrcMusic,:])

    #ずらしながらコサイン類似度算出
    similarityTmp = numpy.zeros([lenSrcMusic, 1, lenYourMusic - lenSrcMusic + 1])
    for i in range(0, lenYourMusic - lenSrcMusic + 1):
        for j in range(0, lenSrcMusic):
            #秒ごとに多次元配列化
            similarityTmp[j,0,i] = (numpy.dot(yourMusic[i+j-1,:], srcMusic[j,:])) / (normYourMusic[i+j-1,0] * normSrcMusic[j,0])

    similarity = numpy.zeros([lenYourMusic - lenSrcMusic + 1, 1])
    for k in range(0, lenYourMusic - lenSrcMusic + 1):
        similarity[k,0] = numpy.sum(similarityTmp[:,0,k]) / lenSrcMusic
...
#サンプル側の最頻を配列化
sampleMusicArray = numpy.array([amen_freq, fourbeat_freq, kikokiko_freq, blueFunk_freq])
#print sampleMusicArray

#ハイパスとローパスを足し合わせる
yourMusic_highPass = numpy.zeros([yourMusic_row, yourMusic_collum, len(sampleMusicArray)])
yourMusic_lowPass = numpy.zeros([yourMusic_row, yourMusic_collum, len(sampleMusicArray)])
yourMusic_highPlusLow = numpy.zeros([yourMusic_row, yourMusic_collum, len(sampleMusicArray)])
