...
[y, ~] = audioread(fullfile(dpath, fname));

csvfilename = [dpath fname '_wavelet_64.csv'];
...
%         result(index, :) = spectrum - mean(spectrum(:,1)); %秒ごとにスペクトラムを記録。平均を引いて標準化。
%         index = index + 1;
%     end

    %ここからwavelet部分
    merge_emphasis = merge_emphasis.';
    scalef = 1:1:2;
    waven = 'gaus5';
    figure;
    subplot(2,1,1);
    result = cwt(merge_emphasis, scalef, waven, 'abslvl');
%     result = result.';
    %ここまでwavelet部分
